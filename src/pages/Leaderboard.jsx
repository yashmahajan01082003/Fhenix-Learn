  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const formattedLeaders = rows.map((leader) => {
          const badges = (() => {
            if (Array.isArray(leader.badges)) return leader.badges;
            if (typeof leader.badges === 'string') {
              try {
                return JSON.parse(leader.badges || '[]');
              } catch {
                return [];
              }
            }
          ...
          return {
            ...
            display_name: leader.display_name ?? `User ${String(leader.user_id ?? '').slice(0, 6)}...`,
            xp: Number(leader.xp) || 0,
            badges,
          };
        });
        ...
        setError('Unable to load leaderboard data. Please refresh or check the API server.');
      } finally {
        setLoading(false);
      }
    };
